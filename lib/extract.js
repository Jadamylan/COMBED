...
async function extractBook({ apiKey, apiUrl, model, pages, images, year, timeoutMs = 240000 }) {
  ...
  pageList.forEach((pg, i) => {
    ...
    if (!parts.length || parts.length > 2 || parts.some((p) => !p || !ALLOWED_MEDIA.has(p.media_type) || typeof p.data !== 'string' || p.data.length < 100)) {
      const e = new Error(`Image ${i + 1} is not a readable JPG, PNG, WEBP, or GIF.`); e.image_index = i; throw e;
    }
  });
